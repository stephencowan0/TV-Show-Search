...
*   For this project you will be using the TV Maze API. Here is the link: [TV Maze API](https://www.tvmaze.com/api)
...
*   You will be using the `https://www.tvmaze.com/api#show-search` endpoint. You will be searching for tv shows by their title. Please use the appropriate parameter to make the request. The [API documentation](https://www.tvmaze.com/api#show-search) has the required information for the parameter.
*   Sample test data - [shows]: girls, Friends.
...
*   A textbox that takes the search text as input. Here, it would be the name of the tv show.
...
*   Upon receiving the JSON response from the API, you will populate the div on the webpage that include the following information about the TV show:
    *   TV show poster
    *   Name
    *   Genres
    *   Summary
...
*   The text field for the name of the TV show should be autopopulated and should be readonly.
...
*   To accomplish the above task you need to:
    ...
    *   Create a table within the database that has `id, tv_show, review, review_date` columns
...
*   The review page should have the following:
    *   A table with the information about the name of the tv show, the review and the date the review was stored in the database.
